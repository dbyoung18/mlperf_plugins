...
std::vector<float> compute_lstm_weights_scales(
  ...
  // per-channel
  for (int i = 0; i < w_ih.size(0); i++) {
    weights_scales.push_back(max_s8 /
	std::max(w_ih[i].abs().max().item<float>(),
		 w_hh[i].abs().max().item<float>())
    );
  }
...
std::vector<at::Tensor> prepack_lstm_weights (
    ...
    const c10::optional<at::Scalar>& data_scale,
    const c10::optional<at::Scalar>& data_shift) {
  ...
  if (data_scale) {
    ...
    primitive_attr attr;
    attr.set_rnn_data_qparams(data_scale.value().toFloat(), data_shift.value().toFloat());
    attr.set_rnn_weights_qparams(weights_mask, weights_scales);
    ////attr.set_output_scales(0, {DNNL_RUNTIME_F32_VAL});
    try {
      lstm_pd = lstm_forward::primitive_desc(lstm_desc, attr, g_cpu());
    } catch (error &e) {
      if (e.status == dnnl_unimplemented) {
	std::cout << "No int8 LSTM implementation is available for this platform.\n";
	throw;
      } else {
        throw;
      }
    ...
    lstm_pd = lstm_forward::primitive_desc(lstm_desc, g_cpu());
  }
...
std::vector<at::Tensor> lstm(
  ...
  int64_t num_layers = all_weights.size();
  auto layer_x = x.contiguous();
  // TODO: add contiguous check for hx & cx
  std::vector<at::Tensor> layer_output;
  ...
  for (int64_t layer = 0; layer < num_layers; layer++) {
    ...
    auto layer_weights = all_weights[layer];
    if (layer_weights.size() == 4) {
      layer_output = lstm_layer(layer_x, layer_hx, layer_cx,
          layer_weights[0], layer_weights[1],
          layer_weights[2], layer_weights[3],
	  data_scale, data_shift);
    } else {
      layer_output = lstm_layer(layer_x, layer_hx, layer_cx,
          layer_weights[0], layer_weights[1],
...
std::vector<at::Tensor> lstm_layer(
  ...
  auto prepacked_weights = prepack_lstm_weights(x, hx_, cx_,
      w_ih, w_hh, bias, data_scale, data_shift);
  ...
  auto key = concat(
    ...
    (bool)data_scale, (bool)data_shift
  );
